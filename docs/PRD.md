...
- `<식별번호>` 허용 범위(확정): `숫자 + 영문 0~10글자`
  - 대소문자 무시(IMG/img 허용)
- 권장 정규식:
  - `^(?i)IMG_(?P<id>\d+[a-z]{0,10})$`
...
- 결정성을 위해 파일 목록 정렬 및 동일 규칙 적용을 유지한다.
...
- ExifTool 호출 오버헤드로 성능이 과도하게 저하되지 않도록 **예방 방안**을 적용한다(§6).
...
### 6.1 권장 전략(최소 1개 이상 적용, 권장: 6.1-1 + 6.1-2 조합)
1) **배치 추출(Chunk)**
   - 200~1000개 단위로 exiftool을 호출하여 JSON을 일괄 추출
   - 파이썬에서 매칭/규칙 적용 및 복사 수행

2) **ExifTool stay_open(지속 프로세스)**
   - exiftool을 1회만 띄우고 명령을 지속 주입하여 프로세스 생성 오버헤드 제거

3) **태그 최소화**
   - 필요한 태그(촬영일 후보 태그 3개 + Make/Model + 파일명 등)만 요청

4) **I/O 동시성 제한**
   - 복사 작업은 워커 풀로 병렬화하되, 동시성(예: 2~4)을 제한하여 디스크 병목/락을 회피
...
2) (추출 단계) ExifTool로 촬영일/카메라/필요 태그 추출(배치 또는 stay_open)  
3) (판별 단계)
...
- ExifTool 번들 포함은 포맷 커버리지 측면에서 안정적이지만,
  성능 이슈는 §6의 예방 전략으로 반드시 완화한다.
